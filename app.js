...
// Charts: Chart.js v4
// Version: v21 (fixed selectors, strings, events)
// ===============================
...
async function init() {
  wireAuthUI();
  // wireOverviewUI(); // (removed: not defined)
  wireLedgerUI();
  wireEditModal();
  wireBankrollModal();
  wireCalendarUI();
...
function wireAuthUI() {
  ...
  supabase.auth.onAuthStateChange(async (_evt, session) => {
    ...
    if ($("#signout")) $("#signout").style.display = State.user ? "" : "none";
...
function renderBar(canvasSel, key, { labels, values }, isPct = false) {
  ...
  State.charts[key] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: { display: false },
        tooltip: { callbacks: { label: c => isPct ? `${c.parsed.y.toFixed(1)}%` : fmtEur(c.parsed.y) } }
      },
...
function runMasaniello() {
  ...
  const b = odds - 1;
  // const edge = Math.max(0, (b*p - (1-p))); // (kept for potential use)
  ...
  $("#ms-nextwin").textContent = fmtEur(cap0 + (rows[0]?.stake || 0) * b || 0);
  $("#ms-nextloss").textContent = fmtEur(cap0 - (rows[0]?.stake || 0) || 0);
  ...
  tb.querySelectorAll("[data-ms-apply]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (rows[i]) { rows[i].cap = newCap; const next = tb.querySelector(`[data-ms-cap="${i + 1}"]`); if (next) next.textContent = fmtEur(newCap); }
    });
  });
