...
(function () {
  ...
  // --- Always-available fallbacks so buttons respond even if init fails
  window.__go = (t) => {
  ...
  window.__clearActive = () => {
    ...
    const el = document.getElementById('current-bk');
    if (el) el.textContent = 'None selected';
  };
  ...
  async function init() {
    ...
    function renderAnalytics() {
      const bk = currentBk();
      if (!bk) { fillAnalyticsEmpty(); return; }
      const bets = State.bets.filter((b) => b.bankroll_id === bk.id);
      const settled = bets.filter((b) => ["win","loss","void"].includes(b.result));
    ...
    function fillAnalyticsEmpty() {
      ["#an-net-profit","#an-staked","#avg-stake","#avg-odds","#an-winrate","#an-pf","#max-dd","#edge"]
        .forEach((id)=> ($(id).textContent = id==="#avg-odds" ? "0.00" : id==="#an-winrate" ? "0%" : "€0"));
    ...
    function pnlByMonth(bets) {
      const map = {};
      for (const b of bets) { const m = (b.date || "").slice(0, 7); if (!m) continue; map[m] = (map[m] || 0) + profitOf(b); }
    ...
    function renderROI() {
      const bk = currentBk();
      if (!bk) { ["#roi-overall","#roi-settled","#roi-profit","#roi-stake"].forEach((id)=> ($(id).textContent = "0")); $("#roi-tbody").innerHTML = ""; return; }
      const bets = State.bets.filter((b) => b.bankroll_id === bk.id);
      const settled = bets.filter((b) => ["win","loss","void"].includes(b.result));
    ...
    function wireCalendarUI() {
      $("#cal-prev")?.addEventListener("click", () => {
        const c = State.calendar; c.month--; if (c.month < 0) { c.month = 11; c.year--; } renderCalendar();
      });
      $("#cal-next")?.addEventListener("click", () => {
        const c = State.calendar; c.month++; if (c.month > 11) { c.month = 0; c.year++; } renderCalendar();
      });
      $("#clear-filter")?.addEventListener("click", () => {
        $("#day-tbody").innerHTML = ""; $("#day-selected").textContent = "—"; $("#day-pnl").textContent = "€0"; renderCalendar();
      });
    }
    ...
    function renderBarSimple(canvasId, label, labels, values) {
      const ctx = document.getElementById(canvasId); const key = canvasId;
      if (!ctx) return;
      if (State.charts[key]) State.charts[key].destroy();
      State.charts[key] = new Chart(ctx, {
        type: "bar",
    ...
    function loadMasanielloList() {
      ...
      sel.onchange = () => {
        ...
        $("#ms-n").value = it.n;
        $("#ms-p").value = it.p; $("#ms-p-range").value = it.p;
    ...
    function ensureShareBox(bk) {
      ...
      if (!box) {
        ...
        $("#share-enable").addEventListener("click", async () => {
          ...
          $("#share-link").value = link; await copyToClipboard(link);
          alert("Share link copied to clipboard.");
        });
      }
      const linkInput = $("#share-link");
      if (bk?.public_slug && bk?.published) linkInput.value = buildPublicLink(bk.public_slug);
