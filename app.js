// QUANTARA — robust build: all clicks work even if Chart.js loads late.
// Still includes the interactive bankroll tooltip + vertical crosshair.
...
let filterDateISO = null;       // exact day filter (from Calendar)
let selectedCalendarISO = null; // selected day on Calendar
let currentMonth = new Date();  // month displayed in Calendar
...
// -------- Chart helpers (all guarded) --------
function haveChart(){ return !!window.Chart; }

function tryRegisterPlugins(){
  if (!haveChart()) return;
  // Data labels
  try { if (window.ChartDataLabels) Chart.register(window.ChartDataLabels); } catch {}
  ...
  try { Chart.register(hoverVLinePlugin); } catch {}
}
...
function lineGradient(ctx){
  ...
  grad.addColorStop(0,"rgba(34,211,238,0.55)"); grad.addColorStop(1,"rgba(124,58,237,0.20)");
  return grad;
}
...
window.addEventListener("DOMContentLoaded", () => {
  tryRegisterPlugins(); // register only if Chart is available
...
async function render(){
  try{
    ...
    console.error(e);
    alert("Runtime error (open DevTools → Console for details).");
  }
}
...
// -------- Ledger month tabs WITH P/L pills --------
...
function drawBankrollChart(){
  if(!haveChart()) return; // UI still works if Chart.js failed to load
    ...
    options:{
      ...
      plugins:{
        ...
        tooltip:{
          ...
          callbacks:{ title:(items)=>items[0]?.label||"", label:(ctx)=>" "+euro(ctx.parsed.y) }
        },
        datalabels:{
