...
console.log("Quantara app v12 — syntax fix");
...
async function sendMagic() {
  ...
  const out = await supabase.auth.signInWithOtp({
    email,
...
async function loadBets() {
  ...
  allBets = (r.data || []).map((x) => {
    let pr = 0;
    if (x.result === "win") pr = (Number(x.odds) - 1) * Number(x.stake);
...
async function createBankroll(e) {
  ...
  if (!name) {
    alert("Give your bankroll a name");
    return;
  }
  const sess = await supabase.auth.getSession();
  ...
  const ins = await supabase
    .from("bankrolls")
    .insert({ name, start_amount: isNaN(start) ? 0 : start, user_id: uid })
    .select("*")
    .single();
  if (ins.error) {
...
async function renameBankroll(id) {
  const b = bankrolls.find((x) => x.id === id);
  if (!b) return;
...
async function saveBankrollStart() {
  const active = getActive();
  if (!active) {
    alert("Pick a bankroll first on Home.");
    return;
  }
  const v = Number($("bankroll-start").value || "0");
  const upd = await supabase
    .from("bankrolls")
    .update({ start_amount: isNaN(v) ? 0 : v })
    .eq("id", active.id);
  if (upd.error) {
...
async function addBetSubmit(e) {
  ...
  const active = getActive();
  if (!active) {
    alert("Select a bankroll first on Home.");
    return;
  }
  ...
  const payload = {
    ...
    event_date: $("f-date").value
      ? new Date($("f-date").value).toISOString()
...
function renderKPIs() {
  ...
  const pf = (() => {
    const gw = settled.filter((b) => b.profit > 0).reduce((s, b) => s + b.profit, 0);
    const gl = Math.abs(
      settled.filter((b) => b.profit < 0).reduce((s, b) => s + b.profit, 0)
    );
    return gl ? gw / gl : 0;
  })();
...
function buildMonthTabs() {
  ...
  allBtn.innerHTML =
    '<span>All</span><span class="month-pill ' +
    (totalPnL >= 0 ? "pos" : "neg") +
    '">' +
    euroShort(totalPnL) +
    "</span>";
...
function renderLedger() {
  ...
  rows.forEach((b) => {
    ...
    tr.innerHTML =
      ...
      "<button class='action-btn action-edit' onclick='window.__edit(" +
      JSON.stringify(b.id) +
      ")'>Edit</button>" +
      "<button class='action-btn action-del'  onclick='window.__del(" +
      JSON.stringify(b.id) +
      ")'>Delete</button>" +
      "</td>";
...
async function onEditSubmit(e) {
  ...
  const payload = {
    event_date: $("e-date").value
      ? new Date($("e-date").value).toISOString()
      : new Date().toISOString(),
  ...
  if (upd.error) {
    alert("Update failed: " + upd.error.message);
    return;
  }
  closeEdit();
  await loadBets();
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          enabled: true,
          displayColors: false,
          callbacks: { label: (c) => " " + euro(c.parsed.y) },
        },
      },
      scales: {
...
function drawPnlBySport(rows) {
  ...
  pnlBySportChart = new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        x: { ticks: { color: "#93a0b7" }, grid: { color: "rgba(147,160,183,0.08)" } },
        y: { ticks: { color: "#93a0b7" }, grid: { display: false } },
      },
    },
  });
}
function drawWinRateBySport(rows) {
  const c = $("winRateBySportChart");
  if (!window.Chart || !c) return;
  const ctx = c.getContext("2d");
  ...
  settled.forEach((b) => {
    ...
    rec.t += 1;
    if (b.result === "win") rec.w += 1;
    bySport.set(b.sport, rec);
  });
  const labels = Array.from(bySport.keys());
  const values = labels.map((s) => {
    const r = bySport.get(s);
    return r.t ? (r.w / r.t) * 100 : 0;
  });
  if (winRateBySportChart) {
    try {
      winRateBySportChart.destroy();
    } catch (_) {}
  }
  winRateBySportChart = new Chart(ctx, {
    type: "bar",
    data: {
      labels,
      datasets: [
    ...
    options: {
      ...
      scales: {
        ...
        y: { ticks: { color: "#93a0b7" }, grid: { color: "rgba(147,160,183,0.1)" }, suggestedMin: 0, suggestedMax: 100 },
      },
    },
  });
}
function drawPnlMonthChart(rows) {
  const c = $("pnlMonthChart");
  if (!window.Chart || !c) return;
  const ctx = c.getContext("2d");
  const map = new Map();
  rows.forEach((b) => {
    const k = b.date.slice(0, 7);
    map.set(k, (map.get(k) || 0) + b.profit);
  });
  const labels = Array.from(map.keys()).sort();
  const values = labels.map((k) => Number((map.get(k) || 0).toFixed(2)));
  if (pnlMonthChart) {
    try {
      pnlMonthChart.destroy();
    } catch (_) {}
  }
  // handle empty dataset safely
  if (labels.length === 0) {
    pnlMonthChart = new Chart(ctx, {
      type: "bar",
      data: {
        labels: ["—"],
        datasets: [
      ...
      options: {
        ...
        scales: {
          y: { beginAtZero: true, suggestedMax: 1, ticks: { color: "#93a0b7" }, grid: { color: "rgba(147,160,183,0.1)" } },
          x: { ticks: { color: "#93a0b7" }, grid: { display: false } },
        },
      },
    });
  ...
  weekdayChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        {
          label: "P&L (€)",
          data: sums.map((x) => Number(x.toFixed(2))),
          backgroundColor: "rgba(124,58,237,0.5)",
          borderColor: "#7c3aed",
        },
      ],
...
function drawOddsHistogram(rows) {
  ...
  const counts = bins.map((r) => {
    const lo = r[0],
      hi = r[1];
    return rows.filter((b) => b.odds >= lo && b.odds < (hi || 1e9)).length;
  });
  if (oddsHistChart) {
    try {
      oddsHistChart.destroy();
    } catch (_) {}
  }
...
function drawResultsPie(rows) {
  const c = $("resultsPieChart");
  if (!window.Chart || !c) return;
  const ctx = c.getContext("2d");
  const counts = { win: 0, loss: 0, pending: 0, void: 0 };
  rows.forEach((b) => {
    counts[b.result] = (counts[b.result] || 0) + 1;
  });
  if (resultsPieChart) {
    try {
  ...
  resultsPieChart = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [
        { data: [counts.win, counts.loss, counts.pending, counts.void], backgroundColor: ["#16a34a", "#ef4444", "#64748b", "#9333ea"] },
      ],
    },
...
function renderROI() {
  ...
  Object.keys(bySport).forEach((sport) => {
    ...
    tr.innerHTML =
      ...
      euro(agg.stake) +
      "</td><td class='right " +
      (roiPct >= 0 ? "profit-pos" : "profit-neg") +
      "'>" +
      euro(agg.profit) +
      "</td><td class='right " +
      (roiPct >= 0 ? "profit-pos" : "profit-neg") +
...
function initTools() {
  ...
  const btnRisk = $("tool-btn-risk"),
    btnPois = $("tool-btn-poisson"),
    btnMasa = $("tool-btn-masa");
  const activate = (k) => {
  ...
  syncRange("rk-prob", "rk-prob-range");
  syncRange("rk-cap", "rk-cap-range");
  ...
  $("rk-run").onclick = () => {
    ...
    const kelly =
      odds > 1 ? clamp(((odds - 1) * p - (1 - p)) / (odds - 1), -1, 1) : 0;
    ...
    const s1 = B * 0.01,
      s2 = B * 0.02,
      s3 = B * 0.03,
      sh = B * Math.max(0, kelly / 2),
      sc = B * Math.max(0, Math.min(cap, kelly));
    const labels = ["Flat 1%", "Flat 2%", "Flat 3%", "Half-Kelly", "Capped Kelly"];
    const data = [s1, s2, s3, sh, sc].map((x) => Number(x.toFixed(2)));
    if (rkStakeChart) {
      try {
        rkStakeChart.destroy();
      } catch (_) {}
    }
    const ctx = $("rkStakeChart").getContext("2d");
    rkStakeChart = new Chart(ctx, {
      type: "bar",
      data: {
        labels,
        datasets: [
  ...
  $("ps-run").onclick = () => {
    ...
    ps1x2Chart = new Chart($("ps1x2Chart").getContext("2d"), {
      ...
      options: {
        ...
        plugins: { legend: { display: false } },
        scales: {
          y: { suggestedMax: 100, ticks: { color: "#93a0b7" }, grid: { color: "rgba(147,160,183,0.1)" } },
          x: { ticks: { color: "#93a0b7" }, grid: { display: false } },
        },
      },
    });
    ...
    const G = 10;
    const Pg = new Array(G + 1).fill(0);
    Pg[0] = Math.exp(-lamT);
    for (let k = 1; k <= G; k++) Pg[k] = (Pg[k - 1] * lamT) / k;

    if (psGoalsChart) {
      try {
        psGoalsChart.destroy();
      } catch (_) {}
    }
    psGoalsChart = new Chart($("psGoalsChart").getContext("2d"), {
      type: "bar",
      data: {
        labels: [...Array(G + 1)].map((_, i) => String(i)),
        datasets: [
          {
            label: "%",
            data: Pg.map((x) => x * 100),
    ...
    [
      ...
      ["Away", pA],
      ["Over " + line, pOver],
      ["Under " + line, Math.max(0, 1 - pOver)],
      ["BTTS Yes", pBTTS],
      ["BTTS No", Math.max(0, 1 - pBTTS)],
    ].forEach(([label, p]) => {
      const tr = document.createElement("tr");
  ...
  $("ms-load").onchange = (e) => {
    if (e.target.value) masaLoadSystem(e.target.value);
  };
...
/* ---------- MASANIELLO ---------- */
function masaComputeRequiredWins(N, p) {
  return Math.max(1, Math.ceil(p * N));
}
...
function masaRecalcHeadsUp() {
  if (!masaState) {
    $("ms-q").textContent = "—";
    $("ms-rem").textContent = "—";
    $("ms-stake").textContent = "—";
    $("ms-nextwin").textContent = "—";
    $("ms-nextloss").textContent = "—";
    return;
  }
  const qreq = masaComputeRequiredWins(masaState.N, masaState.p);
  const remaining = Math.max(0, qreq - masaState.wins);
  const stake = masaNextStake(masaState);
  $("ms-q").textContent = String(qreq);
...
function masaRenderRows() {
  ...
  for (let i = 0; i < masaState.N; i++) {
    ...
    if (i < masaState.history.length) {
      stake = h.stake;
      result = h.result ? h.result.toUpperCase() : "—";
      cap = euro(h.after || masaState.capital);
      action = "";
      ...
      cap = "—";
      action =
        `<button class="action-btn" data-i="${i}" data-r="win">WIN</button>` +
  ...
  tb.querySelectorAll("button[data-i]").forEach((btn) => {
    btn.onclick = () => {
      ...
      const res = btn.getAttribute("data-r");
      if (idx !== masaState.history.length) return;
...
function masaSaveSystem() {
  if (!masaState) {
    alert("Start the sequence first.");
    return;
  }
...
function masaPopulateLoadList(selectId) {
  ...
  sel.innerHTML =
    ...
    list.map((s) => `<option value="${s.id}" ${selectId === s.id ? "selected" : ""}>${s.name}</option>`).join("");
...
function masaLoadSystem(id) {
  ...
  $("ms-capital").value = s.capital;
  $("ms-target").value = s.target;
  $("ms-odds").value = s.odds;
  $("ms-n").value = s.N;
  $("ms-p").value = (s.p * 100).toFixed(1);
  $("ms-p-range").value = s.p * 100;
...
function masaDeleteSystem() {
  const id = $("ms-load").value;
  if (!id) {
    alert("Choose a saved system in the dropdown.");
    return;
  }
  let list = JSON.parse(localStorage.getItem(MASA_KEY) || "[]");
  list = list.filter((s) => s.id !== id);
  localStorage.setItem(MASA_KEY, JSON.stringify(list));
  if (masaState && masaState.id === id) {
    masaState = null;
    localStorage.removeItem(MASA_ACTIVE_KEY);
    $("ms-rows").innerHTML = "";
    masaRecalcHeadsUp();
  }
  masaPopulateLoadList();
