...
let filterDateISO = null;              // optional day filter
let selectedCalendarISO = null;        // clicked day on calendar
let activeMonthKey = null;             // e.g. "2025-08" or null for All
...
/* Gradients & donut shadow (for charts) */
function lineGradient(ctx){ const {ctx:g, chartArea} = ctx.chart; if(!chartArea) return "#7c3aed";
  const grad = g.createLinearGradient(chartArea.left, chartArea.top, chartArea.right, chartArea.bottom);
  grad.addColorStop(0, "rgba(34,211,238,0.35)"); grad.addColorStop(1, "rgba(124,58,237,0.10)"); return grad; }
function ringGradient(ctx, idx){ const c = ctx.chart; const { ctx:g, chartArea } = c; if(!chartArea) return "#22d3ee";
  const palettes = [["#22d3ee","#7c3aed"],["#34d399","#0ea5e9"],["#f472b6","#8b5cf6"],["#fde047","#22d3ee"],["#f97316","#22c55e"]];
  const pair = palettes[idx % palettes.length]; const grad = g.createLinearGradient(chartArea.left, chartArea.top, chartArea.right, chartArea.bottom);
  grad.addColorStop(0, pair[0]); grad.addColorStop(1, pair[1]); return grad; }
const donutShadow = { id:"donutShadow", beforeDatasetDraw(chart){ if(chart.config.type!=="doughnut") return;
...
/* -------- Tabs -------- */
function setTab(tab){
  const blocks = { overview:$("tab-overview"), analytics:$("tab-analytics"), roi:$("tab-roi"), calendar:$("tab-calendar") };
  const buttons = { overview:$("tab-btn-overview"), analytics:$("tab-btn-analytics"), roi:$("tab-btn-roi"), calendar:$("tab-btn-calendar") };
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  $("add-form").addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      selection:emptyNull("f-selection"), odds:parseFloat($("f-odds").value||"1.80"), stake:parseFloat($("f-stake").value||"100"),
      result:$("f-result").value, notes:null
    };
...
async function render(){
  ...
  allBets = (data||[]).map(r => ({
    id:r.id, date:(r.event_date||"").slice(0,10), sport:r.sport||"", league:r.league||"",
    market:r.market||"", selection:r.selection||"", odds:Number(r.odds)||0, stake:Number(r.stake)||0,
    result:r.result, profit: r.result==="win" ? (Number(r.odds)-1)*Number(r.stake) : r.result==="loss" ? -Number(r.stake) : 0
  }));
...
function buildMonthTabs(){
  ...
  allBtn.addEventListener("click", ()=>{ activeMonthKey=null; renderLedger(); buildMonthTabs(); });
  wrap.appendChild(allBtn);

  // Render months newest first
  Object.keys(groups).sort().reverse().forEach(key => {
    ...
    btn.addEventListener("click", ()=>{ activeMonthKey = (activeMonthKey===key? null : key); renderLedger(); buildMonthTabs(); });
...
function drawBankrollChart(){
  ...
  let cum = bankrollStart; const labels = [], data = [];
  ...
  bankrollChart = new Chart(ctx, {
    ...
    options:{ responsive:true, plugins:{legend:{display:false}}, scales:{ x:{ticks:{color:"#93a0b7"}, grid:{color:"rgba(147,160,183,0.1)"}}, y:{ticks:{color:"#93a0b7"}, grid:{color:"rgba(147,160,183,0.1)"}} } }
  });
}
...
function drawAnalyticsStakeChart(){
  const wrap = q(".donut-wrap-lg"); const canvas = $("analyticsStakeChart"); const ctx = canvas.getContext("2d");
  canvas.width = wrap.clientWidth; canvas.height = wrap.clientHeight;
  const bySport = {}; allBets.forEach(b => { bySport[b.sport]=(bySport[b.sport]||0)+b.stake; });
  const labels = Object.keys(bySport); const values = Object.values(bySport);
  if (analyticsStakeChart) analyticsStakeChart.destroy();
  analyticsStakeChart = new Chart(ctx,{ type:"doughnut", data:{ labels, datasets:[{ data:values, borderWidth:1, borderColor:"#0d1524", backgroundColor:c=>ringGradient(c,c.dataIndex), hoverOffset:6 }] }, options:{ responsive:false, maintainAspectRatio:false, plugins:{legend:{labels:{color:"#e7eefc"}}}, cutout:"76%", radius:"70%" }, plugins:[donutShadow] });
}
function drawPnlBarChart(){
  ...
  const labels=daily.map(d=>d.date); const values=daily.map(d=>Number(d.pnl.toFixed(2)));
  if(pnlBarChart) pnlBarChart.destroy();
  pnlBarChart=new Chart(ctx,{ type:"bar", data:{labels, datasets:[{label:"P&L (€)", data:values}]}, options:{ responsive:true, plugins:{legend:{display:false}}, scales:{ x:{ticks:{color:"#93a0b7"}, grid:{display:false}}, y:{ticks:{color:"#93a0b7"}, grid:{color:"rgba(147,160,183,0.1)"}} } });
}
function drawOddsHistogram(){
  const ctx=$("oddsHistChart").getContext("2d");
  const bins=[[1,1.5],[1.5,2],[2,2.5],[2.5,3],[3,10]]; const labels=["1–1.5","1.5–2","2–2.5","2.5–3","3+"]; 
  const counts=bins.map(([lo,hi])=> allBets.filter(b=> b.odds>=lo && b.odds<(hi||1e9)).length);
  if(oddsHistChart) oddsHistChart.destroy();
  oddsHistChart=new Chart(ctx,{ type:"bar", data:{labels, datasets:[{label:"Bets", data:counts}]}, options:{ responsive:true, plugins:{legend:{display:false}}, scales:{ x:{ticks:{color:"#93a0b7"}, grid:{display:false}}, y:{ticks:{color:"#93a0b7"}, grid:{color:"rgba(147,160,183,0.1)"}, beginAtZero:true, precision:0} } });
}
function drawResultsPie(){
  ...
  resultsPieChart=new Chart(ctx,{ type:"doughnut", data:{ labels:["win","loss","pending","void"], datasets:[{ data:[counts.win,counts.loss,counts.pending,counts.void], backgroundColor:c=>ringGradient(c,c.dataIndex), borderWidth:1, borderColor:"#0d1524" }]}, options:{ plugins:{legend:{labels:{color:"#e7eefc"}}}, cutout:"65%" }, plugins:[donutShadow] });
}

/* -------- Calendar (P/L in cells) -------- */
...
function renderROI(){
  ...
  const staked=settled.reduce((s,b)=> s+b.stake,0); const profit=settled.reduce((s,b)=> s+b.profit,0);
  ...
  const tbody=$("roi-tbody"); tbody.innerHTML=""; Object.entries(bySport).forEach(([sport,agg])=>{ const roiPct=agg.stake? (agg.profit/agg.stake)*100 : 0;
    const tr=document.createElement("tr"); tr.innerHTML=`<td>${sport}</td><td class="right">${agg.bets}</td><td class="right">${euro(agg.stake)}</td><td class="right ${agg.profit>=0?'profit-pos':'profit-neg'}">${euro(agg.profit)}</td><td class="right ${roiPct>=0?'profit-pos':'profit-neg'}">${roiPct.toFixed(2)}%</td>`; tbody.appendChild(tr); });
}

/* -------- Utils -------- */
function groupByDateSum(items){ const map=new Map(); items.forEach(({date,pnl})=> map.set(date,(map.get(date)||0)+pnl));
  return Array.from(map.entries()).sort((a,b)=> a[0].localeCompare(b[0])).map(([date,pnl])=>({date,pnl})); }
function computeMaxDrawdown(){ const sorted=[...allBets].sort((a,b)=> a.date.localeCompare(b.date));
  let equity=bankrollStart, peak=bankrollStart, maxDD=0; sorted.forEach(b=>{ equity+=b.profit; peak=Math.max(peak,equity); const dd=peak-equity; if(dd>maxDD) maxDD=dd; }); return maxDD; }
