// app.js — QUANTARA
// Full, refactored, feature-complete client (Overview, Analytics, ROI, Calendar)
...
let filterDateISO = null;       // exact day filter (set from Calendar)
let selectedCalendarISO = null; // currently selected day on Calendar
let currentMonth = new Date();  // month being displayed in Calendar
...
function ringGradient(ctx, idx){
  ...
  const palettes = [
    ["#22d3ee","#7c3aed"], ["#34d399","#0ea5e9"], ["#f472b6","#8b5cf6"],
    ["#fde047","#22d3ee"], ["#f97316","#22c55e"]
  ];
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  $("add-form").addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      odds: parseFloat($("f-odds").value || "1.80"), stake: parseFloat($("f-stake").value || "100"),
...
async function render(){
  ...
  // ensure profile row exists (RLS friendly)
  await supabase.from("profiles").upsert({ id: session.user.id });

  // read bets via the safe view
...
function drawCalendar(){
  ...
  for(let i=0;i<42;i++){
    ...
    cell.addEventListener("click", ()=>{
      ...
      filterDateISO = selectedCalendarISO; // drives Overview ledger if you switch
      drawCalendar();
      updateDayBox();
    });
...
function updateDayBox(){
  ...
  rows.forEach(b=>{
    const tr=document.createElement("tr");
    const cls=b.profit>=0?"profit-pos":"profit-neg";
    tr.innerHTML=`<td>${b.sport}</td><td>${b.market}</td><td>${b.selection}</td>
      <td class="right">${b.odds.toFixed(2)}</td><td class="right">€${b.stake.toFixed(2)}</td>
      <td class="right">${b.result}</td><td class="right ${cls}">€${b.profit.toFixed(2)}</td>`;
    tbody.appendChild(tr);
  });
}
...
function renderROI(){
  ...
  Object.entries(bySport).forEach(([sport,agg])=>{
    ...
    const tr=document.createElement("tr");
    tr.innerHTML=`<td>${sport}</td><td class="right">${agg.bets}</td>
