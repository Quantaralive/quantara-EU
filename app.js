...
// ---- Supabase setup (singleton to avoid multi-client warning)
...
// reuse one client per window
const supabase =
  globalThis.__supabaseClient ||
  createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
    auth: { persistSession: true, storageKey: "quantara-auth-v1" },
  });
...
// ---- Boot
console.log("Quantara boot", VERSION);
window._QUANTARA_HEALTH = "js-loaded-" + VERSION;
...
async function init() {
  ...
  // tiny visibility: attach quick helpers for debugging if needed
  window.__whoami = async () => {
    const { data } = await supabase.auth.getSession();
    console.log("whoami:", data.session?.user || null);
  ...
  window.__smoke = async () => {
    try {
      ...
      const bks = await supabase.from("bankrolls").select("id").limit(1);
      console.log("bankrolls select ok?", !bks.error);
      const bets = await supabase.from("bets").select("id").limit(1);
      console.log("bets select ok?", !bets.error);
      return { session: !!s.data.session, bankrolls: !bks.error, bets: !bets.error };
    } catch (e) {
      console.error(e);
...
function wireAuthUI() {
  ...
  // Sign out (legacy button + menu later)
...
// Small helper: swap the top-right auth area between inputs and user menu
...
async function loadEverything() {
  ...
  if (
    ...
    !State.bankrolls.find((b) => b.id === State.activeBankrollId)
  ) {
    ...
    div.innerHTML = `
      <div class="bankroll-row">
        <div>
          ...
          <div class="bankroll-sub">Start: ${fmtEur(
            bk.start_amount || 0
          )} · Bets: <span data-bk-count="${bk.id}">—</span></div>
        </div>
...
// Buttons in HOME (NO inline)
$("#btn-new-bankroll")?.addEventListener("click", () => {
...
function renderOverview() {
  ...
  const settled = bets.filter(
    (b) => b.result === "win" || b.result === "loss" || b.result === "void"
  );
...
function renderMonthTabs(bets) {
  ...
  for (const [k, arr] of Object.entries(groups)) {
    const label = new Date(k + "-01").toLocaleDateString(undefined, {
      year: "numeric",
      month: "long",
    });
...
function renderLedger(bets) {
  ...
  for (const b of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="right" style="color:${p >= 0 ? "#16a34a" : "#ef4444"}">${fmtEur(
        p
      )}</td>
...
function renderAnalytics() {
  ...
  const avgOdds = bets.length
    ? bets.reduce((s, b) => s + (Number(b.odds) || 0), 0) / bets.length
    : 0;
  const wins = settled.filter((b) => b.result === "win").length;
  const wr = settled.length ? (wins / settled.length) * 100 : 0;
  const pf = (() => {
...
function fillAnalyticsEmpty() {
  [
    ...
    $(id).textContent = id === "#avg-odds" ? "0.00" : id === "#an-winrate" ? "0%" : "€0"
  );
...
function renderROI() {
  ...
  if (!bk) {
    ["#roi-overall", "#roi-settled", "#roi-profit", "#roi-stake"].forEach(
      (id) => ($(id).textContent = id === "#roi-overall" ? "0%" : "0")
    );
    $("#roi-tbody") && ($("#roi-tbody").innerHTML = "");
    return;
  }
  const bets = State.bets.filter((b) => b.bankroll_id === bk.id);
  const settled = bets.filter(
...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    cell.innerHTML = `<div class="date-num">${d}</div><div class="amt" style="color:${color}">${
      amt === 0 ? "" : fmtEur(amt)
    }</div>`;
...
function runMasaniello() {
  ...
  tb.querySelectorAll("[data-ms-apply]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (rows[i]) {
        rows[i].cap = newCap;
        const next = tb.querySelector(`[data-ms-cap="${i + 1}"]`);
        if (next) next.textContent = fmtEur(newCap);
      }
    });
  });
