...
// Register chart datalabels if present
...
let activeMonthKey = null;      // "YYYY-MM" or null (All)
let filterDateISO = null;       // day filter when coming from Calendar
...
const donutShadow = {
  id:"donutShadow",
  beforeDatasetDraw(chart){ if(chart.config.type!=="doughnut") return; const ctx=chart.ctx; ctx.save(); ctx.shadowColor="rgba(0,0,0,0.35)"; ctx.shadowBlur=14; ctx.shadowOffsetY=8; },
...
function setTab(tab){
  ...
  blocks[tab].classList.remove("hidden"); buttons[tab].classList.add("active");
...
function buildMonthTabs(){
  ...
  btnAll.className = "month-tab" + (activeMonthKey===null ? " active" : "");
  btnAll.textContent = "All";
  btnAll.addEventListener("click",()=>{ activeMonthKey=null; filterDateISO=null; renderLedger(); buildMonthTabs(); });
  wrap.appendChild(btnAll);

  // Group dates by YYYY-MM
  const set = new Set(allBets.map(b => b.date.slice(0,7)));
  Array.from(set).sort().reverse().forEach(ym => {
    const b = document.createElement("button");
    b.className = "month-tab" + (activeMonthKey===ym ? " active" : "");
    b.textContent = monthName(ym);
    b.addEventListener("click",()=>{ activeMonthKey = (activeMonthKey===ym ? null : ym); filterDateISO=null; renderLedger(); buildMonthTabs(); });
    wrap.appendChild(b);
  });
}
  ...
  bankrollChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label(ctx){ return " " + euro(ctx.parsed.y); }
          }
        },
        datalabels: {
          align: "top",
          ...
          display: (ctx) => {
            ...
            // Show value on last point always, and on hovered points
            return i === last || ctx.active;
          }
        }
      },
      scales:{
